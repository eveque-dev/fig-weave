  ...
  useEffect(() => {
    Promise.all([
      document.fonts.load(
        ...
        "原脚本与重放辅助函数保留图画好了还在补字体新罗马换图例放左下角字号再小一点为了几处改动又来回聊轮把最后交给鼠标运行绘脚本直接编辑形保留细节调整修改不只面里支持随导出次能重现实际结果前后偏移数值便于阅读已四舍五入从发着熟悉工作流完成范围各引擎不同以对应台为准少多线体验默认中文鼠尾草绿",
      ),
      document.fonts.load(
        '750 96px "Noto Sans SC Variable"',
        "图画好了还在补把最后几步交给鼠标拖图例调字号修改不只留在画面里从绘图脚本出发少多一点直接",
      ),
    ])
