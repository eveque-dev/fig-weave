...
第三版（2026-09-26）：32 秒、1920 × 1080、30 fps，曜石黑背景。
采用独立设计的“代码与图形编织”视觉，只有原创合成音效，无配乐、无人声。

![代码画图，网页改图。](../assets/figweave/promo-cover-v3.png)

开场以代码和真实图形的连接展开；中段演示实际 ggplot2 编辑与 R 脚本导出；
结尾介绍三个在线入口及它们各自的输出格式。封面是单独设计的概念插画。

| 时间 | 内容 |
| --- | --- |
| 0–8 秒 | 代码与真实图表通过编织线条连接；点出字体、字号、图例的零散要求 |
| 8–10.5 秒 | 「打开网页，接着改图」 |
| 10.5–20 秒 | 真实 ggplot2 图表：12 pt → 10 pt，图例右侧 → 图内左下方 |
| 20–26 秒 | 实际导出 R 代码的节选与修改后的图表并置 |
| 26–29 秒 | Matplotlib、Plotly / pyecharts、ggplot2 三个网页入口与导出格式 |
| 29–32 秒 | FigWeave 品牌、官网入口与派生关系 |

## 声音设计

10 类原创程序合成音色、27 个同步事件：低频启动、机械按键、编织连接、干燥点击、
...
FFmpeg 将 960 帧和音效编码为 MP4，不改变动画时间轴。
...
- `src/scenes/`：六段逐帧动画；指针与选框按实测几何重建。
