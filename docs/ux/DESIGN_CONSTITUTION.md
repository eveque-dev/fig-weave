...
`index.css` 声明，由色块与页面共同消费；背景选择仅在当前页面生效，每次打开默认曜石黑，不进文档。
只调整应用底、输入框底和工作区底；白色图纸、图内属性和导出颜色不受影响。

## FigWeave 在线工作台（2026-09-26）

用户要求黑底白字。在线默认曜石黑，覆盖背景、面板、输入框与文字等语义 token；
每次打开页面默认黑色，旧背景偏好不覆盖默认值。桌面默认值不变，图表画纸与导出颜色不跟随 UI 主题。
...
2026-09-26 用户指定参考 Tavotto 官网的滚动叙事：首页图表舞台固定，滚动进度驱动工作台展开与图表变更。此展示场景可随滚动缩放，区别于工作台控件的 4px 动效限制；不拦截滚轮，减少动态效果时关闭固定滚动与缩放，使用章节按钮。
