 ...
 * 编码 Agent 的图标。
 *
 * **只用本地矢量**（lucide 的线性图标或内联 SVG，无底块），不加载任何远程图片。
 * 区分靠形状与名称，名称本身就是可访问文本。
 *
...
/** Claude 标志（本地内联 SVG，保留品牌橙 #D97757；外框无底块、无边框）。 */
...
/** OpenAI 标志（Simple Icons 9.x，CC0；本地内联 SVG，跟随文字色）。 */
...
const GLYPHS: Record<string, Glyph> = {
  // Codex 用 OpenAI 标志（2026-09-11 用户要求）
  codex: OpenAiGlyph,
  // Claude 用其标志的本地内联 SVG（2026-09-10 改）；表外的 key 落到 Bot，靠名称文本区分。
  claude: ClaudeGlyph,
}
...
export function AgentIcon({
  ...
  return (
    <span
      ...
      className={cn(
        'flex shrink-0 items-center justify-center text-ink-2',
        className,
      )}
    >
