...
export function DragOverlay({ objects, disabled, move }: { objects: RObject[]; disabled: boolean; move: (id: string, dx: number, dy: number) => void }) {
  const { t } = useTranslation('dialogs')
  const svg = useRef<SVGSVGElement>(null)
  const gesture = useRef<{ id: string; x: number; y: number; width: number; height: number } | null>(null)
  ...
  const down = (e: PointerEvent, id: string) => {
    ...
    gesture.current = { id, x: e.clientX, y: e.clientY, width: rect.width, height: rect.height }
    setSelected(id)
  }
  return <svg ref={svg} data-r-drag-overlay viewBox="0 0 1000 1000" preserveAspectRatio="none" className="r-drag-overlay" style={{ pointerEvents: disabled ? 'none' : undefined }}>
    {objects.map((object) => {
      ...
      const props = {
        ...
        onPointerDown: (e: PointerEvent) => down(e, object.id),
        onPointerMove: (e: PointerEvent) => { const g = gesture.current; if (g) setOffset({ id: g.id, x: (e.clientX - g.x) / g.width, y: (e.clientY - g.y) / g.height }) },
        onPointerUp: (e: PointerEvent) => {
          const g = gesture.current; gesture.current = null; setOffset(null)
          if (g && Math.hypot(e.clientX - g.x, e.clientY - g.y) >= 2) move(g.id, (e.clientX - g.x) / g.width, (e.clientY - g.y) / g.height)
        },
        onPointerCancel: () => { gesture.current = null; setOffset(null) },
        onKeyDown: (e: React.KeyboardEvent) => {
          const step = e.shiftKey ? .02 : .005
          const delta: Record<string, [number, number]> = { ArrowLeft: [-step, 0], ArrowRight: [step, 0], ArrowUp: [0, -step], ArrowDown: [0, step] }
          if (delta[e.key] && !disabled) { e.preventDefault(); move(object.id, ...delta[e.key]) }
